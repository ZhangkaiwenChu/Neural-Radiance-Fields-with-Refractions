...
__global__ void add3(
	...
	input[elem_idx * input_stride + dim_idx] = other[elem_idx * other_stride + dim_idx];
}

template <typename T>
__global__ void add32(
	const uint32_t n_elements,
	const uint32_t input_stride,
	const uint32_t other_stride,
	T* __restrict__ input,
	...
	input[elem_idx * input_stride + dim_idx] = (T)other[elem_idx * other_stride + dim_idx];
}

template <typename T>
__global__ void set3(
	const uint32_t n_elements,
	const uint32_t input_stride,
	const uint32_t other_stride,
	const T* __restrict__ input,
	T* __restrict__ other
) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
	...
	other[elem_idx * input_stride + dim_idx] = input[elem_idx * other_stride + dim_idx];
}
...
public:
	...
	NerfNetwork(uint32_t n_pos_dims, uint32_t n_dir_dims, uint32_t n_extra_dims, uint32_t dir_offset, const json& pos_encoding, const json& dir_encoding, const json& density_network, const json& rgb_network, const json& offset_encoding, const json& offset_network) : m_n_pos_dims{n_pos_dims}, m_n_dir_dims{n_dir_dims}, m_dir_offset{dir_offset}, m_n_extra_dims{n_extra_dims} {
		...
		m_offset_encoding.reset(tcnn::create_encoding<T>(n_pos_dims + n_dir_dims, offset_encoding, density_network.contains("otype") && (tcnn::equals_case_insensitive(density_network["otype"], "FullyFusedMLP") || tcnn::equals_case_insensitive(density_network["otype"], "MegakernelMLP")) ? 16u : 8u));


		json local_offset_network_config = offset_network;
		local_offset_network_config["n_input_dims"] = m_offset_encoding->padded_output_width();
		local_offset_network_config["n_output_dims"] = n_pos_dims;
		m_offset_network.reset(tcnn::create_network<T>(local_offset_network_config));
	}
	...
	void inference_mixed_precision_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, bool use_inference_params = true) override {
		...
		tcnn::GPUMatrixDynamic<T> offset_network_input{ m_offset_encoding->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };
		tcnn::GPUMatrixDynamic<T> offset_network_output{ m_offset_network->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };

		tcnn::GPUMatrixDynamic<float> temp{ m_offset_network->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };

//		printf("%d,%d,%d\n", m_n_pos_dims , m_rgb_network->output_width(),m_rgb_network->padded_output_width());

		m_offset_encoding->inference_mixed_precision(
			stream,
			input.slice_rows(0, m_n_pos_dims + m_n_dir_dims),
			offset_network_input,
			use_inference_params
		);

		m_offset_network->inference_mixed_precision(stream, offset_network_input, offset_network_output, use_inference_params);

		tcnn::linear_kernel(add3<T>, 0, stream,
			batch_size * 3,
			offset_network_output.m(),
			input.m(),
			offset_network_output.data(),
			input.data()
		);

		//printf("%d,%d,%d\n", offset_network_output.m(), input.slice_rows(0, m_pos_encoding->input_width()).m(), m_pos_encoding->input_width());
		

		m_pos_encoding->inference_mixed_precision(
			stream,
			offset_network_output.slice_rows(0, m_pos_encoding->input_width()),
	...
	std::unique_ptr<tcnn::Context> forward_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>* output = nullptr, bool use_inference_params = false, bool prepare_input_gradients = false) override {
		...
		forward->offset_network_input = tcnn::GPUMatrixDynamic<T>{ m_offset_encoding->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };
		...
		forward->density_encoding_input = tcnn::GPUMatrixDynamic<T>{ m_n_pos_dims, batch_size, stream, m_pos_encoding->preferred_output_layout() };

		forward->offset_encoding_ctx = m_offset_encoding->forward(
			stream,
			input.slice_rows(0, m_offset_encoding->input_width()),
			&forward->offset_network_input,
			use_inference_params,
			prepare_input_gradients
		);

		forward->offset_network_output = tcnn::GPUMatrixDynamic<T>{ m_offset_network->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };
		forward->offset_network_ctx = m_offset_network->forward(stream, forward->offset_network_input, &forward->offset_network_output, use_inference_params, prepare_input_gradients);

		forward->density_encoding_input.initialize_constant(0.f);
		
		tcnn::linear_kernel(add3<T>, 0, stream,
			batch_size * 3,
			forward->density_encoding_input.m(),
			forward->offset_network_output.m(),
			forward->density_encoding_input.data(),
			forward->offset_network_output.data()
		);
		tcnn::linear_kernel(add3<T>, 0, stream,
			batch_size * 3,
			forward->density_encoding_input.m(),
		...
		forward->pos_encoding_ctx = m_pos_encoding->forward(
			stream,
			forward->density_encoding_input,
	...
	void backward_impl(
		...
		tcnn::GPUMatrixDynamic<T> dL_dpos_encoding_input;
		tcnn::GPUMatrixDynamic<T> dL_dpadded_pos_enc;
		// Backprop through pos encoding if it is trainable or if we need input gradients
		if (dL_ddensity_network_input.data()) {
			dL_dpadded_pos_enc = tcnn::GPUMatrixDynamic<float>{ m_offset_network->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };

			if (dL_dinput) {
				dL_dpos_encoding_input = dL_dinput->slice_rows(0, m_pos_encoding->input_width());
			}
			else {
				dL_dpos_encoding_input = tcnn::GPUMatrixDynamic<T>{ m_pos_encoding->input_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };
			}

			//printf("%d,%d\n", dL_ddensity_network_input.data() ? dL_ddensity_network_input.m():0, dL_dpos_encoding_input.data() ? dL_dpos_encoding_input.m():0);
			...
			tcnn::linear_kernel(set3<T>, 0, stream,
				batch_size * 3,
				dL_dpos_encoding_input.m(),
				dL_dpadded_pos_enc.m(),
				dL_dpos_encoding_input.data(),
				dL_dpadded_pos_enc.data()
			);

			tcnn::GPUMatrixDynamic<float> dL_doffset_input;
			if (dL_dinput) {
				dL_doffset_input = tcnn::GPUMatrixDynamic<T>{ m_offset_encoding->padded_output_width(), batch_size, stream, m_offset_encoding->preferred_output_layout() };
			}
			
			m_offset_network->backward(
				stream,
				*forward.offset_network_ctx,
				forward.offset_network_input,
				forward.offset_network_output,
				dL_dpadded_pos_enc,
				dL_dinput ? &dL_doffset_input : nullptr,
				use_inference_params,
				param_gradients_mode
			);
		}
	...
	void set_params_impl(T* params, T* inference_params, T* gradients) override {
		...
		m_offset_encoding->set_params(params + offset, inference_params + offset, gradients + offset);
		offset += m_offset_encoding->n_params();

		m_offset_network->set_params(params + offset, inference_params + offset, gradients + offset);
		offset += m_offset_network->n_params();
	}
	...
	void initialize_params(tcnn::pcg32& rnd, float* params_full_precision, float scale = 1) override {
		...
		m_offset_encoding->initialize_params(rnd, params_full_precision, scale);
		params_full_precision += m_offset_encoding->n_params();

		m_offset_network->initialize_params(rnd, params_full_precision, scale);
		params_full_precision += m_offset_network->n_params();
	}

	size_t n_params() const override {
		return m_pos_encoding->n_params() + m_density_network->n_params() + m_dir_encoding->n_params() + m_rgb_network->n_params() + m_offset_encoding->n_params() + m_offset_network->n_params();
	}
	...
	std::vector<std::pair<uint32_t, uint32_t>> layer_sizes() const override {
		...
		auto offset_layers = m_offset_network->layer_sizes();
		layers.insert(layers.end(), rgb_layers.begin(), rgb_layers.end());
		layers.insert(layers.end(), offset_layers.begin(), offset_layers.end());
		return layers;
	}
...
private:
	...
	std::unique_ptr<tcnn::Network<T>> m_offset_network;
	std::shared_ptr<tcnn::Encoding<T>> m_pos_encoding;
	std::shared_ptr<tcnn::Encoding<T>> m_dir_encoding;
	std::shared_ptr<tcnn::Encoding<T>> m_offset_encoding;
	...
	struct ForwardContext : public tcnn::Context {
		...
		tcnn::GPUMatrixDynamic<float> temp;

		std::unique_ptr<Context> pos_encoding_ctx;
		std::unique_ptr<Context> dir_encoding_ctx;
		std::unique_ptr<Context> offset_encoding_ctx;

		std::unique_ptr<Context> density_network_ctx;
		std::unique_ptr<Context> rgb_network_ctx;

		tcnn::GPUMatrixDynamic<T> density_encoding_input;
		tcnn::GPUMatrixDynamic<T> offset_network_input;
		tcnn::GPUMatrixDynamic<T> offset_network_output;
		std::unique_ptr<Context> offset_network_ctx;
	};
};
